...
Lsensor2 = adafruit_vl6180x.VL6180X(mux[4])
...
def get_start_angle(dist, dist_target):
    ...
    theta = Lsensor.range - (Lsensor2.range + 2)

    error_angle = theta_target - theta

    start_angle = Kp_angle * error_angle
    print('start angle limitless: {0}'.format(start_angle))
    start_angle = limit(start_angle, -0.3, 0.3)
    
    print('Theta Target: {0}'.format(theta_target))
    print('dist: {0}'.format(dist))
    print('error_theta: {0}'.format(error_theta))
    print('start angle: {0}'.format(start_angle))
    print('error_angle: {0}'.format(error_angle))
...
def run_control_loop(dist_target):
    ...
    start_angle, error_angle = get_start_angle(Lsensor.range, 2) #distance from wall
...
def forward():
    ...
    if Rsensor.range < 30:
        ...
        ledR.value = False
    reset_odometry()
    while abs(run_control_loop(100)[1]) > 3: #target is 120 mm before stopping
        time.sleep(0.02)
    reset_controls()
...
def smallforward():
    ...
    if Rsensor.range < 30:
        ...
        ledR.value = False
    reset_odometry()
    while abs(run_control_loop(30)[1]) > 3: #target is 120 mm before stopping
        time.sleep(0.02)
    reset_controls()
    right_motor.duty_cycle, left_motor.duty_cycle = 0, 0
...
def right():
    ...
    while left_position < 90 or right_position > -90:
        right_motor_dir_1.value = 1 #backwards left
        right_motor_dir_2.value = 0 #forwards

        left_motor_dir_1.value = 0 #backwards right
        ...
        if left_position < 90:
            left_motor.duty_cycle = 15000
            #print("4")
            #print(left_position)
        else:
            left_motor.duty_cycle = 0
        if right_position > -90:
            right_motor.duty_cycle = 15000
            #print("5")
...
def reverse():
    print("reverse")
    right_motor_dir_1.value = 1
    right_motor_dir_2.value = 0

    left_motor_dir_1.value = 1
    left_motor_dir_2.value = 0
    left_enc.position = 0
    right_enc.position = 0
    left_position = left_enc.position
    right_position = right_enc.position
    
    while left_enc.position > -30 or right_enc.position > -30:
        if left_enc.position > -30:
            left_motor.duty_cycle = 35000
        else:
            left_motor.duty_cycle = 0
        if right_enc.position > -30:
...
if __name__ == "__main__":
    while True:
        if Fsensor.range >= 100:
            print(Fsensor.range)
            forward()
            time.sleep(1)
        elif Fsensor.range < 100 and Fsensor.range > 30:
             print(Fsensor.range)
             smallforward()
        elif Fsensor.range < 20:
            reverse()
        if Fsensor.range <= 30 and Lsensor.range < 100:
            right()
