...
def run_control_loop(dist_target):
    ...
    start_angle, error_angle = get_start_angle(Lsensor.range, 200) #distance from wall
...
def smallforward():
    ...
    if Rsensor.range < 30:
        ...
        ledR.value = False
    reset_odometry()
    while abs(run_control_loop(10)[1]) > 3: #target is 120 mm before stopping
        time.sleep(0.02)
    reset_controls()
    right_motor.duty_cycle, left_motor.duty_cycle = 0, 0
...
if __name__ == "__main__":
    while True:
        if Fsensor.range >= 60:
            forward()
            time.sleep(1)
#         elif Fsensor.range < 100 and Fsensor.range > 50:
#             print(Fsensor.range)
#             #smallforward()
        elif Fsensor.range < 20:
            reverse()
        if Fsensor.range < 60 and Lsensor.range < 100 and Rsensor.range > 100:
            right()
